...
export const POST = facadeHandler('recordings.discards.transcript.write', async (ctx) => {
  ...
  // ⚖ NO REFUSAL SWEEP ANY MORE (capture pipeline PR4). This handler used to
  // wrap everything below in a try/catch whose only job was to DELETE the
  // staged object on every route-level refusal: the phone staged a throwaway
  // copy before it posted, and each retry staged another. The audio it names is
  // the take's own FINALIZED object now — there is no throwaway to strand, and
  // a refusal must never destroy the recording it just refused to write words
  // for.
  //
  // Parsed at the DOOR, same placement rule the GET obeys: a malformed body is
  // this caller's 400, never an upstream 502.
