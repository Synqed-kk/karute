...
export default async function ReservationsPage({
  ...
  try {
    const { props, storeKey } = await reservationsProps({ locale, store })
    // ⚖ VIEW STATE IS STORE-SCOPED (the recording page's own precedent). A
    // `?store=` navigation keeps the same screen instance, so the open rail
    // card, the picked slot, the lit chip and the selected row would all survive
    // a lens switch — one store's decision standing over another store's list.
    return <ReservationsScreen key={storeKey} {...props} />
  } catch {
