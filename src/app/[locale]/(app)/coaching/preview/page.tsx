...
// ─────────────────────────────────────────────────────────────
// /[locale]/coaching/preview — DEMO PAGE (sample data)
// ─────────────────────────────────────────────────────────────
// Renders the real coaching screens (DataDrivenStaffView, DataDrivenOwnerRoi) with
// HARDCODED sample data, so the design can be seen in the running app before the
// backend is wired. SAFE: reads no real data, writes nothing, touches no DB — it is
// a purely static display. A dev/demo affordance on a preview branch, not a
// production surface (no route links to it; open it directly).
...
import { DataDrivenStaffView } from '@/components/coaching/redesign/DataDrivenStaffView'
import { DataDrivenOwnerRoi } from '@/components/coaching/redesign/DataDrivenOwnerRoi'

const CATS: (CategoryScore & { label: string })[] = [
...
const CLOSING_TREND: MetricPoint[] = [
...
const SAMPLE_STAFF: StaffCoachingView = {
  scope: 'staff-self',
  metrics: {
    closingRate: 0.62,
    rebookingRate: 0.71,
  ...
  findings: [
    {
      ...
      headline: '提案を急いでいる',
      impact: '12回中8回、お客様が要望を言い終える前に提案に入っています。そのうち5回が不成約でした。',
      recommendation: 'お客様が話し終えるまで、あと一呼吸おいてから提案する。「他に気になるところは？」を一つ挟むと成約が伸びています。',
    ...
    },
    {
      ...
      headline: '価格の根拠が弱い',
      impact: '価格提示の場面7回のうち5回で、そのお客様の目的と結びつけずに金額だけをお伝えしています。',
      recommendation: '金額の前に「〇〇を良くするため」と、その方が言った目的に一言つなげる。',
    ...
    },
    {
      ...
      impact: '11回中9回、お客様の言葉を具体的に受けとめてから次に進めています。満足度4.4を支えている強みです。',
      recommendation: 'この受けとめを、価格提示やクロージングの場面でも同じように続ける。',
  ...
  focus: [
    {
      ...
      rationale: '今のあなたに一番効くのはこれ。急がず提案するだけで、先月の傾向では成約が5件増えていた計算です。',
  ...
  learnFromTop: [
    {
      ...
      behavior: '「他に気になるところはありますか？」— 提案の直前にこの一言を挟んでいます。',
      adoptionNote: 'トップ層の9割が実践 ・ 匿名',
    },
    {
      id: 'tp2',
      categoryKey: 'next_step',
      behavior: '次回予約のとき、お客様が最初に話した目的（結婚式・旅行など）に必ず結びつけています。',
...
const treated: MetricPoint[] = CLOSING_TREND
const control: MetricPoint[] = [
...
export default function CoachingPreviewPage() {
  return (
    <div className="mx-auto max-w-3xl space-y-10 p-4 pb-16">
      <div className="rounded-lg border border-amber-300/50 bg-amber-50/60 p-3 text-[12px] text-amber-800 dark:border-amber-400/20 dark:bg-amber-500/[0.07] dark:text-amber-300">
        プレビュー — サンプルデータで表示しています。実データはAnthonyがバックエンドを接続後に反映されます。
      </div>

      <section className="space-y-3">
        <h2 className="text-base font-semibold text-foreground">スタッフ画面</h2>
        <DataDrivenStaffView view={SAMPLE_STAFF} />
      </section>

      <section className="space-y-3">
        <h2 className="text-base font-semibold text-foreground">オーナー画面 — コーチングの効果</h2>
        <DataDrivenOwnerRoi roi={SAMPLE_ROI} />
      </section>
    </div>
  )
}
