...
- RE-ENTRY (highest value): the most recent session's summary/entries may carry 次回 lines — open loops that progress between visits: a pending medical result or appointment the customer mentioned (an MRI, a doctor visit), homework assigned (セルフケア), promises the staff made (「次回は腰を重点的に」「期限を延長します」), deferred proposals, or symptoms to re-check. When present, EACH genuine open loop surfaces as its own todayActions item (within the max-3 cap; a pending medical result outranks homework — ${clinical ? 'e.g. 「MRIの結果を確認」 before 「宿題のハムストレッチの実施状況を確認」' : 'e.g. 「検査の結果を確認」 before 「宿題の実施状況を確認」'}), phrased as an action — and ONLY there: concerns and recommendedFocus must not repeat them. Asking 「MRIは行かれましたか？」 unprompted is the "this place remembers me" moment; a promise the staff forgets is trust lost. Only what the records actually say — never invent.
