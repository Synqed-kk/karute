 ...
 */
export function whoRuleExtractionJa(ctx?: PromptContext, role = '施術者'): string {
  const cleaned = ctx?.customerName ? cleanNameToken(ctx.customerName) : ''
  const name = cleaned ? `${cleaned}様` : 'このお客様'
  // role === 施術者 ⇒ the bodywork family — keep the original v3.2 dialect
  // (verified UltraCode wording) verbatim; every other role gets neutral prose.
  const bw = role === '施術者'
  return `【最重要：誰の事実か】
- 判断基準は「発言者が誰か」ではなく「その事実が誰の体・生活についてか」。${role}の発言でも、お客様の${bw ? '体の状態' : '状態'}・所見・原因の見立て・指導内容（例：${bw ? '「かなり緊張が強い」「ここまで可動域を出したい」' : '「かなり状態が気になる」「ここまで良くしたい」'}）は必ず記録する。
...
- お客様自身の購入・契約（回数券の残回数・有効期限・サブスク・コース）と、それについて${role}がお客様に約束した対応（期限の延長など）は、お客様の事実として必ず記録する。除外するのは店側の売上・実績・キャンペーンの話。
- ${bw ? '施術中' : '接客中'}の電話対応・配達対応・他のスタッフとの業務連絡など、${name}との会話ではないやり取りの内容は一切記録しない。そこに出てくる名前・予約日時・変更は他のお客様のものである。次回予約は本人との会話の中で合意された場合のみ記録する。
- お客様が子供や家族を同伴している場合、同伴者の発言が「お客様」ラベルで記録されることがある。呼びかけ（「ママ」等）・話し方・返事の流れから同伴者の発言と分かるものは、お客様本人の事実として記録しない。
- 文字起こしではお客様の呼び名が誤変換されることがある。本人との${bw ? '施術会話' : '接客会話'}の流れの中で使われる呼び名は、${name}と表記が違っても本人への呼びかけとして扱う。他のお客様の名前と判断するのは、電話・業務連絡など本人との会話でないやり取りに現れる場合のみ。
- 文脈判断の手がかり：一人称（私・僕・俺）が誰を指すか、直前の質問→回答の流れ（誰が誰に聞いたか）、その情報を知り得るのは誰か（店の売上・同僚・${bw ? '施術技術' : '専門技術'}の話＝スタッフ、自分の症状・怪我の経緯・仕事時間・自分が買った回数券＝お客様）。判断は話題のまとまり単位ではなく一文ずつ行う（雑談の最中にも所見・${bw ? '痛み' : '悩み'}の訴えが挟まれる）。
- 迷う場合は、まず上記の手がかりをすべて当てはめて判断する。手がかりで判断できる場合は、話者ラベルが乱れていても必ず記録する。すべて当てはめても「誰のことか」判断できない場合に限り、記録しない（誤った事実は欠落より有害）。
- 話者ラベルは文の途中で誤って入れ替わることがあり、「（周囲の会話・不明）」のラベルも例外ではない。その行が前後のお客様・施術者のやり取りと明らかに一続き（質問への返答、「痛っ」など${bw ? '施術への' : ''}短い反応、文の続き）であれば、本人の発言として扱い抽出に使ってよい。前後と繋がらない独立した会話（隣の席・電話・店内の雑談）にだけ現れる事実は抽出しない。`
}

/** The WHO block for the karute SUMMARY prompt (JA) — compressed 3-bullet form. */
export function whoRuleSummaryJa(ctx?: PromptContext, role = '施術者'): string {
  const cleaned = ctx?.customerName ? cleanNameToken(ctx.customerName) : ''
  const name = cleaned ? `${cleaned}様` : 'お客様本人'
  const bw = role === '施術者'
  return `【最重要：誰の事実か】
- 要約するのは「お客様」に関する内容のみ。判断基準は発言者ではなく「その事実が誰の体・生活についてか」— ${role}がお客様について述べた所見・見立て・指導は必ず含める。お客様自身の回数券・契約・支払いの話もお客様の事実として含める。
...
- 話者ラベルは文の途中で誤って入れ替わることがあり、「（周囲の会話・不明）」も例外ではない。前後のやり取りと明らかに一続きの発言（質問への返答${bw ? '・「痛っ」などの短い反応' : 'や短い反応'}）は本人の発言として扱ってよい。前後と繋がらない独立した会話にだけ現れる事実は使わない。文字起こしでお客様の呼び名が誤変換されることがある — ${bw ? '施術会話' : '接客会話'}の流れの中の呼び名は${name}への呼びかけとして扱う。`
}
