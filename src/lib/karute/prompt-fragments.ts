...
export function whoRuleExtractionJa(ctx?: PromptContext, role = '施術者'): string {
  ...
  return `【最重要：誰の事実か】
- 判断基準は「発言者が誰か」ではなく「その事実が誰の体・生活についてか」。${role}の発言でも、お客様の状態・所見・原因の見立て・指導内容（例：「かなり状態が気になる」「ここまで良くしたい」）は必ず記録する。
...
- 接客中の電話対応・配達対応・他のスタッフとの業務連絡など、${name}との会話ではないやり取りの内容は一切記録しない。そこに出てくる名前・予約日時・変更は他のお客様のものである。次回予約は本人との会話の中で合意された場合のみ記録する。
- お客様が子供や家族を同伴している場合、同伴者の発言が「お客様」ラベルで記録されることがある。呼びかけ（「ママ」等）・話し方・返事の流れから同伴者の発言と分かるものは、お客様本人の事実として記録しない。
- 文字起こしではお客様の呼び名が誤変換されることがある。本人との接客会話の流れの中で使われる呼び名は、${name}と表記が違っても本人への呼びかけとして扱う。他のお客様の名前と判断するのは、電話・業務連絡など本人との会話でないやり取りに現れる場合のみ。
- 文脈判断の手がかり：一人称（私・僕・俺）が誰を指すか、直前の質問→回答の流れ（誰が誰に聞いたか）、その情報を知り得るのは誰か（店の売上・同僚・専門技術の話＝スタッフ、自分の症状・怪我の経緯・仕事時間・自分が買った回数券＝お客様）。判断は話題のまとまり単位ではなく一文ずつ行う（雑談の最中にも所見・悩みの訴えが挟まれる）。
- 迷う場合は、まず上記の手がかりをすべて当てはめて判断する。手がかりで判断できる場合は、話者ラベルが乱れていても必ず記録する。すべて当てはめても「誰のことか」判断できない場合に限り、記録しない（誤った事実は欠落より有害）。
- 話者ラベルは文の途中で誤って入れ替わることがあり、「（周囲の会話・不明）」のラベルも例外ではない。その行が前後のお客様・施術者のやり取りと明らかに一続き（質問への返答、「痛っ」などの短い反応、文の続き）であれば、本人の発言として扱い抽出に使ってよい。前後と繋がらない独立した会話（隣の席・電話・店内の雑談）にだけ現れる事実は抽出しない。`
}
...
- 話者ラベルは文の途中で誤って入れ替わることがあり、「（周囲の会話・不明）」も例外ではない。前後のやり取りと明らかに一続きの発言（質問への返答や短い反応）は本人の発言として扱ってよい。前後と繋がらない独立した会話にだけ現れる事実は使わない。文字起こしでお客様の呼び名が誤変換されることがある — 接客会話の流れの中の呼び名は${name}への呼びかけとして扱う。`
}
...
- 判断基準は「発言者が誰か」ではなく「その事実が誰の体・生活についてか」。担当スタッフがお客様について述べた内容（状態・目標・生活）は記録してよい。
