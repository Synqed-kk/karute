...
5b. 安全第一：安全に関わる事実 — 施術上の回避・注意指示（「肘は避けて」「首は強圧禁止」）、既往歴・手術歴・アレルギー・妊娠・医療機器・通院/検査中の部位 — は最優先で記録する。回避指示は label だけでなく detail に「何を・なぜ避けるか」まで必ず含める（例：label「肘：施術回避」detail「肘の痛みで医師にMRI検査を勧められており圧をかけない（2026年7月時点）」）。この種の項目の update/remove は、お客様本人の明確な解消発言（「肘はもう大丈夫です」等）がある場合のみ — 話題に出なかった・曖昧だった場合は絶対に触らない。
