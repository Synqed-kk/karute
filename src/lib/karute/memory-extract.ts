...
4. 既存項目（どの source・pinned でも）と同じ事実を重複して add しない。label の言い回しが違っても、同じ部位・同じ事象なら同一の事実（例：既存に「ゴルフ肘」がある時、「ゴルフ肘：施術回避」を add するのは重複 — 内容が深まったなら update、その項目が pinned なら何も出さない）。
...
4. Do NOT duplicate an existing item (any source, pinned or not). A different label wording for the same body area / same fact is STILL a duplicate (e.g. with existing "golf elbow", adding "golf elbow: avoid during treatment" is a duplicate — update it if the fact deepened, or emit nothing when that item is pinned).
