...
- 主題の厚み: 出力前に「本日の会話で最も長く・繰り返し扱われた話題（主題）」を特定する。主題は1行に圧縮せず、会話に出た側面ごと（経緯・${bw ? '所見・施術とその反応' : '所見・本日の内容とその反応'}・指導など）にエントリーを分けて厚く記録する — 統合ルールが禁じるのは「同じ側面の重複」であり、主題の側面を分けることではない。一度だけ触れられた話題が主題より厚くなってはいけない。出力順は、安全に関わる情報の直後に主題のエントリーを置き、その後にその他を続ける。
...
- 主題の厚み: 最初のラベル（${firstLabel}）には「本日の会話を最も長く占めた訴え・話題」を先頭に書く — 医学的に目立つ話題や会話の最後に出た話題を機械的に選ばない。主題は内容量に応じて行を分けて厚く書き、一度だけ触れられた話題を主題と同じ厚みにしない。
