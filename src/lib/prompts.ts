...
- 主題の厚み: 出力前に「本日の会話で最も長く・繰り返し扱われた話題（主題）」を特定する。主題は1行に圧縮せず、会話に出た側面ごと（経緯・${bw ? '所見・施術とその反応' : '所見・本日の内容とその反応'}・指導など）にエントリーを分けて厚く記録する — 統合ルールが禁じるのは「同じ側面の重複」であり、主題の側面を分けることではない。統合ルールの「出力直前の見直し」でも、主題の側面エントリー同士は統合しない（主題については、この主題の厚みルールが統合より優先する）。ただし側面ごとに分けても、同じ一文をそのまま複数の category に重複して書いてはならない — 各エントリーはその category の視点（経緯は訴えの言葉、所見は観察と評価、施術・指導は実施内容と反応）で互いに異なる情報を書く。書こうとしている一文が他の category のエントリーと同じになる場合は、より詳しい方の1件にまとめる。生活習慣（仕事時間・睡眠など）が症状のきっかけとして既に他のエントリーに書かれている場合、lifestyle に同じ文言を繰り返さない — 追加の生活情報がある場合のみ別途書く。一度だけ触れられた話題が主題より厚くなってはいけない。出力順は、安全に関わる情報の直後に主題のエントリーを置き、その後にその他を続ける。
