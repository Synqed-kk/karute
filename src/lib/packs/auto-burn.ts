 ...
 * stays unkeyed. There is no client action to mint a key per: a cron tick has
 * no inbound request (see the audit note below), so any key minted here would
 * be fresh on every attempt and dedupe nothing, while a key REUSED across the
 * driver's fallback-to-the-next-pack retry would be actively wrong — the
 * second attempt is a DIFFERENT pack, and core would replay the first pack's
 * stored row instead of burning the one that was chosen. This path is already
 * double-burn-guarded twice over: the driver's own history ledger (guard 1)
 * and the DB's appointment-scoped partial unique index, which works here
 * precisely because a cron burn always has an appointment_id.
