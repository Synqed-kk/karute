...
// deleteStaff now imports the profiles.id → synqed staff.id resolver. Stub it
// so the real staff-map module (which calls unstable_cache at load, unmocked
// here) isn't pulled into this suite's graph. These tests use profile-less ids,
// so the action never actually calls it — the resolver's own coverage lives in
