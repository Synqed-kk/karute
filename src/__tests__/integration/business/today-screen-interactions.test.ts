...
describe('⚖ flag 76 — the 60分配置 rail hears about the rooms', () => {
  ...
  it('the screen has exactly one door to the book, and two ways in through it', () => {
    ...
    // memo it lives in.
    //
    // ⚠ MIGRATED AGAIN at LIVE-WHILE-DRAGGING, 4 → 3, AND THE INVARIANT GOT
    // STRONGER RATHER THAN WEAKER (⚖ ADJUDICATION L2 M-5). The two walks on a
    // FOREIGN board — the escape hatch's and the before-list's — were two builds
    // of the same book for the same array; they are ONE now, behind `bookFor`,
    // which is the only place on this screen that walks the door for a board that
    // is not the one on screen. So the three are the definition, the frame's own
    // `ledger` memo, and that single shared walk. 「ONE door, every walk named」 is
    // what it always was, with one fewer walk to name.
