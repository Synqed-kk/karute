...
describe('⚖ EVERYTHING MOVES — the demo-interaction machinery, run for real', () => {
  ...
  it('the screen commits a section’s values, and says so — once, per section', () => {
    ...
    // while page.tsx has handed over the real save (door ON), which saves to core first.
...
describe('PKT-BUILD-N3-2 §3 H5 — the screen calls the pure save door', () => {
  ...
  it('N3-3 Block calls the live turnover fact and control doors', () => {
    expect(blockSource).toContain('wordsTurnoverFact(section, block.id, values)')
  ...
  it('N3-4 Block calls the live room block and room options doors', () => {
    ...
    expect(blockSource).toContain('{ title: roomBlock.title }')
    ...
    expect(SRC_CODE).toContain('for (const s of Object.values(seedSectionById)) out[s.id] = { ...s, blocks:')
  })
