...
// · a GET never creates the 本店 primary store even when the list is empty
// (the divergence pin — that prelude stays web-only) · POST/PATCH require
// the owner role (roster + resolved Bearer identity) with a standard facade
// 403 on denial, NOT a soft 200 { error } · Idempotency-Key required on
// create, not on update · STORE_LIMIT_REACHED when the entitlement cap is
// armed and reached · exactly one audit row per successful write, and none
// on a denied/failed write · an unknown/out-of-tenant update id writes
// nothing.
...
describe('GET /api/app/v1/stores', () => {
  ...
  it('an empty list never creates the 本店 primary store (the divergence pin — Bearer GET performs no writes)', async () => {
    storesList.mockResolvedValue({ stores: [] })
    const res = await listGET(getReq(), noParams)
    expect(res.status).toBe(200)
