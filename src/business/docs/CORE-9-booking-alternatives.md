...
- Full Jest suite: 563 suites passed, 10,525 tests passed and one skipped with `TZ=UTC`. The initial local-timezone run exposed an unchanged date-label test; it passes in UTC, matching CI.
...
Integrated main at 8596941a before the final run. Its room model makes standard-first/private-last mandatory and removes RoomPolicy; the availability adapter follows that interface.
